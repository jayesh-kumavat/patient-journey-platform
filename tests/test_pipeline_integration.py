...
class TestTransforms:
    ...
    def test_warehouse_row_counts(self, integration_engine):
        tables_to_check = [
            ("dim_patient", 50),
            ("dim_physician", 20),
            ("fact_prescription", 100),
            ("fact_patient_journey", 100),
            ("therapy_switches", 1),
        ]
...
class TestInference:
    ...
    def test_anomaly_results_stored(self, integration_engine, model_dir, monkeypatch):
        ...
        count = df["cnt"].iloc[0]
        assert count > 0, "anomaly_results should have at least 1 row"
        logger.info(f"Stored {count} anomaly results")
...
class TestFullPipelineE2E:
    ...
    def test_end_to_end(self, integration_engine, generated_data, model_dir, monkeypatch):
        ...
        # Verify warehouse tables populated
